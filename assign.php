...
/**
 * Assign roles to users for video directory rol only
 *
...
$PAGE->set_url('/local/video_directory/assign.php');
$PAGE->navbar->add(get_string('pluginname', 'local_video_directory'), new moodle_url('/local/video_directory/'));
$PAGE->navbar->add(get_string('users', 'moodle'));

$contextid = 1;
// Find local_video_directory role id
$local_video_role = $DB->get_field('role', 'id', ['shortname' => 'local_video_directory'], IGNORE_MISSING);
$roleid    = $local_video_role;
$returnurl = optional_param('returnurl', null, PARAM_LOCALURL);

list($context, $course, $cm) = get_context_info_array($contextid);

$url = new moodle_url('/local/video_directory/assign.php', array('contextid' => $contextid));
...
if ($roleid) {
    $a = new stdClass;
    $a->role = 'local_video_directory'; //$assignableroles[$roleid];
...
switch ($context->contextlevel) {
    case CONTEXT_SYSTEM:
        ...
        local_video_admin_externalpage_setup('assignroles', '', array('contextid' => $contextid, 'roleid' => $roleid));
        break;
    case CONTEXT_USER:
...
function local_video_admin_externalpage_setup($section, $extrabutton = '', array $extraurlparams = null, $actualurl = '', array $options = array()) {
